...
function selectRange(from, to) {
  ...
  for (var i = from; i <= to; i++) {
    ...
    if (m && m.role !== 'system') _selectedMsgs[i] = true;
  }
}
...
function selectAllMsgs() {
  const char = activeCharacter();
  const idxs = (char.chat || []).map(function(m, i) { return { m: m, i: i }; }).filter(function(x) { return x.m.role !== 'system'; });
...
function deleteSelected() {
  ...
  idxs.forEach(function(i) {
    if (char.chat[i] && char.chat[i].role !== 'system') char.chat.splice(i, 1);
  });
  saveState();
  exitMultiSelect();
}
